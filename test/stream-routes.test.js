...
test("Responses API 转发时删除不兼容的 store 字段", async () => {
  const server = await makeServer();
  const originalFetch = globalThis.fetch;
  let upstreamBody = null;

  globalThis.fetch = async (url, options) => {
    assert.equal(url, "https://upstream.example/v1/responses");
    upstreamBody = JSON.parse(options.body);
    return new Response(JSON.stringify({ id: "response-test" }), {
      status: 200,
  ...
  try {
    const response = await originalFetch(`${server.url}/v1/responses`, {
      method: "POST",
      headers: {
        authorization: "Bearer global-gateway-key",
        "content-type": "application/json"
      },
      body: JSON.stringify({ model: "gpt-test", input: "hello", store: true })
    });

    assert.equal(response.status, 200);
    assert.deepEqual(upstreamBody, { model: "gpt-test", input: "hello" });
  } finally {
...
test("Chat Completions API 转发时删除不兼容的 store 字段", async () => {
  const server = await makeServer();
  const originalFetch = globalThis.fetch;
  let upstreamBody = null;
  ...
  globalThis.fetch = async (url, options) => {
    ...
    upstreamBody = JSON.parse(options.body);
    return new Response(JSON.stringify({ id: "chat-completion-test" }), {
      status: 200,
      headers: { "content-type": "application/json" }
    });
  };
  ...
  try {
    const response = await originalFetch(`${server.url}/v1/chat/completions`, {
      method: "POST",
      headers: {
        authorization: "Bearer global-gateway-key",
        "content-type": "application/json"
    ...
    assert.deepEqual(await response.json(), { id: "chat-completion-test" });
  } finally {
    globalThis.fetch = originalFetch;
    await server.close();
  }
